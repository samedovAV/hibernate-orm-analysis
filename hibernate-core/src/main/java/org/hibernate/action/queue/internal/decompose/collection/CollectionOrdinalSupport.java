...
/// See [across all collection mutation decomposers.].
///
/// The ordinal scheme uses a two-level hierarchy:
/// <ol>
/// See [ - **Collection Slot**: Each collection operation receives a unique ordinalBase from ActionQueue (0, 1, 2, ...)].
/// 		based on the order it was added to the queue
/// See [ - **Operation Type**: DELETE, UPDATE, INSERT, etc. have fixed offsets within the slot].
/// </ol>
///
...
///
/// See [ - All DELETEs: 5000].
/// See [ - All UPDATEs: 5100].
/// See [ - All INSERTs: 5200].
/// See [ - All WRITEINDEXes: 5300].
///
/// This ensures:
///
/// See [ - No ordinal collisions between operation types].
/// See [ - Correct ordering: DELETE &lt; UPDATE &lt; INSERT &lt; WRITEINDEX].
/// See [ - Works for both bundled (single operation per type) and non-bundled (multiple operations per type)].
/// See [ - Compatible with FlushCoordinator's grouping logic for self-referential tables].
///
...
/// **Limits**:
///
/// See [ - Max collections per flush: Limited by SLOT_SIZE (1000 collections)].
///
...
public final class CollectionOrdinalSupport {
	...
	/// This determines:
	///
	/// See [ - How many collections can be processed in a single flush (up to 1000)].
	/// See [ - The grouping logic in FlushCoordinator (ordinal / SLOT_SIZE groups by collection)].
	///
